...
import { streamViaAppServer, type AppServerApprovalRequest, type ApprovalOutcome } from './app-server.ts'
...
function readCodexCachedModels(connection: CodexConnectionOptions): CodexCachedModel[] {
  const codexHome = resolve(expandHomePath(connection.codexHome ?? '~/.codex'))
  try {
    const document = JSON.parse(readFileSync(join(codexHome, 'models_cache.json'), 'utf8')) as { models?: unknown }
    return Array.isArray(document.models) ? document.models as CodexCachedModel[] : []
  } catch {
    // The cache is optional and can be briefly incomplete while Codex refreshes it.
    return []
  }
}

/** Models offered to Harness's native Models settings discovery UI. */
export function discoverCodexModels(connection: CodexConnectionOptions): LlmDiscoveredModel[] {
  ...
  for (const cached of readCodexCachedModels(connection)) {
    ...
    const contextWindow = positiveSafeInteger(cached.max_context_window)
      ?? positiveSafeInteger(cached.context_window)
    discovered.push({
      id: cached.slug,
      ...(typeof cached.display_name === 'string' && cached.display_name.length > 0
...
const CODEX_REASONING_LEVELS: ReadonlyArray<{ id: ReasoningEffortId; name: string }> = [
  { id: 'low' as ReasoningEffortId, name: '低' },
  { id: 'medium' as ReasoningEffortId, name: '中' },
  { id: 'high' as ReasoningEffortId, name: '高' },
  { id: 'xhigh' as ReasoningEffortId, name: '超高' },
  { id: 'max' as ReasoningEffortId, name: '最高' },
  { id: 'ultra' as ReasoningEffortId, name: '极致' },
]
...
async function findOnPath(command: string): Promise<string | undefined> {
  ...
  for (const directory of (process.env.PATH ?? '').split(delimiter).filter(Boolean)) {
    for (const extension of extensions) {
      ...
      const candidate = join(directory, `${command}${hasExtension ? '' : extension.toLowerCase()}`)
      if (await isExecutable(candidate)) return candidate
      const upperCandidate = join(directory, `${command}${hasExtension ? '' : extension.toUpperCase()}`)
...
export async function resolveCodexBinary(configured?: string): Promise<string | undefined> {
  if (configured === undefined || configured.trim().length === 0) return findOnPath('codex')
...
function stderrSummary(stderr: string, stdout: string): string {
...
type CodexChild = ChildProcessWithoutNullStreams

function killProcessTree(child: CodexChild, timeoutMs = 3_000): Promise<void> {
...
function parseUsage(event: Record<string, unknown>): CodexUsage | undefined {
  const usage = record(event.usage)
  if (usage === undefined) return undefined
  const totalInput = numberField(usage, 'input_tokens')
  const outputTokens = numberField(usage, 'output_tokens')
  if (totalInput === undefined || outputTokens === undefined) return undefined
...
function describeItem(eventType: string, item: Record<string, unknown>): string | undefined {
  const itemType = stringField(item, 'type')
  if (itemType === 'reasoning') return stringField(item, 'text')
  if (itemType === 'command_execution') {
    const command = stringField(item, 'command')
    if (eventType === 'item.started') return command === undefined ? '正在执行命令\n' : `正在执行命令：${command}\n`
...
function waitForProcess(child: CodexChild, signal?: AbortSignal): Promise<ProcessResult> {
...
export class CodexAdapter extends LlmAdapter {
  ...
  override resolveModel(provider: string, model: string, _signal?: AbortSignal): Promise<LlmResolvedModelInfo> {
    ...
    const contextWindow = configured === undefined ? undefined : resolveModelContextWindow(connection, configured)
    return Promise.resolve({
      ...
      reasoning: {
        ...
        defaultEffort: (this.config.options().defaultReasoningEffort ?? 'low') as ReasoningEffortId,
      },
    })
  }
  ...
  async * stream(options: GenerateOptions): AsyncIterable<StreamChunk> {
    ...
    const cwd = connection.cwd === undefined ? process.cwd() : resolve(expandHomePath(connection.cwd))
    if (this.config.requestApproval !== undefined && options.sessionId !== undefined) {
      try {
    ...
    const contextWindow = configuredModel === undefined
      ? undefined
      : resolveModelContextWindow(connection, configuredModel)
    ...
    const startCodex = (args: string[]): CodexChild => {
    ...
    try {
      ...
      let actualUsage: CodexUsage | undefined
